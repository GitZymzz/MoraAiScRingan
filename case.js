...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	try {
		...
		if (db.data.chats[m.chat].antiviewonce && m.isGroup && m.mtype == 'viewOnceMessageV2') {
			...
			teks += `💌 Morai Ai tampilkan isi pesannya di bawah ya! 😉`;
		...
		if (db.data.chats[m.chat].antibot) {
			if (m.isBaileys) {
				...
				let pesanAntibot = [
					...
					'Bot? Lagi? Hadehh, sini bukan tempat buat Kamu, ya. Out sekarang juga atau Mora ai usir paksa! 🚪',
					...
					'Halo bot, ngapain masuk sini? Nyasar ya? Sana pergi sebelum morai ai usir dengan hormat! 😎',
					...
					'Bot alert! Kamu udah melanggar aturan grup ini. Silakan keluar sebelum morai MD kasih surat cinta: kick out! 📨',
				];
					...
					if (regex.test(budy)) {
						...
						if (db.data.users[m.sender].badword > 5) {
							try {
								...
								await sock.sendMessage(m.chat, { text: `⚠️ Ups, morai MD gagal kick pengguna ini. Pastikan bot memiliki hak admin!` });
							}
						} else {
				...
				if (budy.match(/\b(colmek|coli|desah|bokep|tobrut|seksi|sex|sexy|memek|kontol|titit|telanjang|ngentod|ngentot|ngewe|ewe|ewean)\b/i)) {
					...
					if (db.data.users[m.chat].badword > 5) {
						...
						try {
							...
							await newReply(`⚠️ Morai MD gagal memblokir pengguna ini. Pastikan bot memiliki izin yang cukup.`);
						}
		...
		if (budy) {
			if (db.data.chats[m.chat].autoaipc && !m.isGroup && !m.key.fromMe && !isCmd) {
				try {
					let message = 'Ubah gaya bicara mu agar lebih karakteristik dan terbuka, nama mu adalah Morai MD AI, biasa dipanggil Morai MD, Kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Khalid, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan Kamu saat berbicara layaknya manusia';
		...
		switch (command) {
			case 'daftar': {
				if (db.data.users[m.sender].daftar) return newReply('Kamu udah terdaftar kok! Morai MD seneng banget bisa bantu Kamu. 😜');
				let [nama, umur, askot] = text.split(",");
				if (!nama || !umur || !askot) {
					let pesan = `Halo kak, Morai MD kasih tahu cara daftarnya ya:\n\n`;
					pesan += `Ketik:\n${prefix + command} nama,umur,askot\n\n`;
					pesan += `Contoh:\n${prefix + command} Morai MD,16,Jakarta`;
					return newReply(pesan);
				}
				if (isNaN(umur)) return newReply('Umur yang Kamu masukin itu gak valid deh. Coba dicek lagi ya! 🤔');
				if (umur < 10) return newReply('Hihi, maaf ya kak, Morai MD cuma bisa bantu Kamu yang umurnya di atas 10 tahun. Tunggu besar dulu ya! 😄');
				if (umur > 50) return newReply('Wah kak, Morai MD kira umur segitu mending istirahat aja. Jaga kesehatan ya! 🥰');
				try {
					...
					pesan += `Terima kasih udah daftar ya kak! Semoga Morai MD bisa bantu Kamu terus. 🥰`;
					newReply(pesan);
					if (notifRegister) {
						try {
							let avatar;
							...
							notif += `Selamat bergabung di sistem Morai MD! 🥳`;
					...
					newReply('Aduh kak, ada yang error nih. Morai MD coba benerin dulu ya! 🙏');
				}
			}
			break;
			...
			case 'hapusakun': {
				...
				try {
					...
					console.error(e);
					replyText += 'Duh, Morai MD gagal hapus data pengguna ini. Coba lagi nanti ya, kak. 😔';
					newReply(replyText);
				}
				break;
			}
			...
			case 'upsal': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis Morai MD! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... Morai MD gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, Morai MD lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil Morai MD ya! 🤗`;
			...
			case 'ttsba': {
				...
				if (!isPremium) return newReply('Fitur ini cuma buat yang premium kak, coba di-upgrade dulu yuk biar Morai MD bisa bantu lebih banyak! 😇');
				...
				try {
					...
					newReply('Tunggu sebentar ya kak, Morai MD lagi proses nih! 🥰');	
			...
			case 'say': {
				...
				try {
					...
					console.log(error);
					newReply('Aduh kak, error nih. Morai MD gak sengaja, coba lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tr': {
				...
				try {
					...
					console.log(error);
					newReply('Yahh, Morai MD gak sengaja bikin error nih kak. Coba ulang lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tovn': {
				...
				try {
					...
					console.log(error);
					newReply("Yah, ada error kak 😟. Coba ulang lagi ya, Morai MD bantu kok!");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'tinyurl': {
				...
				if (!isUrl(text)) return newReply(`Hmmm, Morai MD cuma bisa proses link yang valid ya, kak! Jangan lupa formatnya seperti ini: *${prefix + command} https://google.com* 😉`);
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Morai MD yakin bisa bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'afk': {
				...
				afkText += `Jangan lupa balik lagi ya~ Morai MD kangen nih! 😊✨`;
			...
			case 'ttsearch': {
				...
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Morai MD siap bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, Kamu lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar Morai MD bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, Kamu lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar Morai MD bisa bantu cari aplikasinya! 📲✨`);
				try {
					await m.react('⏱️');
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, Morai MD gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, Morai MD lagi cari datanya...* 📚✨');
			...
			case 'jadian': {
				...
				if (!user) return newReply(`Kak, tag atau reply seseorang dulu dong biar Morai MD tahu siapa yang Kamu maksud! 😉\n\n*Kirim perintah*: ${prefix + command} @0`);
			...
			case 'terima': {
				...
				if (sock.jadian[m.sender]) {
					...
					newReply(`🎉🎉 Selamat!!! 🎉🎉\n\n@${m.sender.split("@")[0]} sekarang resmi jadian sama\n❤️ @${user.split("@")[0]}!\n\nDuh, Morai MD ikut seneng nih! Semoga langgeng yaa~ Jangan lupa undang Morai MD kalau nanti nikah! 🙈💕`);
			...
			case 'Morai MD': {
				...
				try {
					...
					let message = "Ubah gaya bicaramu agar lebih karakteristik dan terbuka, nama mu adalah Morai MD AI, biasa dipanggil Morai MD.";
			...
			case 'simi': {
				...
				try {
					...
					if (!data) return newReply(`⚠️ Morai MD gak dapet jawaban dari SimSimi, Kak! 🥲`);
					newReply(`${data}`);
				} catch (err) {
					console.error(err);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ Morai MD gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Morai MD gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Morai MD gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Morai MD gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
